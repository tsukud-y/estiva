...
this.strings={};this.chars={};$.extend(this.strings,bootstrapAmznJQ.strings);$.extend(this.chars,bootstrapAmznJQ.chars);}();$(window).load(function(){amznJQ.windowOnLoad();});amznJQ.declareAvailable('JQuery');amznJQ.declareAvailable('jQuery');if(bootstrapAmznJQ){;$.each(bootstrapAmznJQ._l,function(){amznJQ.addLogical(this[0],this[1])});$.each(bootstrapAmznJQ._s,function(){amznJQ.addStyle(this[0])});$.each(bootstrapAmznJQ._d,function(){amznJQ.declareAvailable(this[0],this[1])});$.each(bootstrapAmznJQ._a,function(){amznJQ.available(this[0],this[1])});$.each(bootstrapAmznJQ._o,function(){amznJQ.onReady(this[0],this[1])});}
...
var timerID=null;hoverSet.bind('mouseover.amzPopover',function(e){if(!popover&&!timerID){timerID=setTimeout(function(){if(!popover){if(!settings.triggeringEnabled||settings.triggeringEnabled.call(triggers)){popover=displayPopover(settings,triggers,function(){popover=null;});}}
...
popover.appendTo(rootElement());position(popover,settings,triggers);$('input[type=text]').blur();if(settings.group){if(openGroupPopover[settings.group]){openGroupPopover[settings.group].close();}
