...
/*     Set initial residual. */
    ...
    dcopy_(n, &b[1], &c__1cgs, &work[r * work_dim1 + 1], &c__1cgs);
    if (dnrm2_(n, &x[1], &c__1cgs) != 0.) {
	(*matvec)(&c_b5, &x[1], &c_b6, &work[r * work_dim1 + 1]);
	if (dnrm2_(n, &work[r * work_dim1 + 1], &c__1cgs) <= tol) {
	    goto L30;
	}
    }

    bnrm2 = dnrm2_(n, &b[1], &c__1cgs);
    if (bnrm2 == 0.) {
	bnrm2 = 1.;
    }
...
/*     Here we choose RTLD = R. */
    ...
    dcopy_(n, &work[r * work_dim1 + 1], &c__1cgs, &work[rtld * work_dim1 + 1], &
	    c__1cgs);
...
/*     Perform Conjugate Gradient Squared iteration. */
    ...
    rho = ddot_(n, &work[rtld * work_dim1 + 1], &c__1cgs, &work[r * work_dim1 + 
	    1], &c__1cgs);
    if (abs(rho) < rhotol) {
	goto L25;
    }
...
/*           Compute U. */
	...
	beta = rho / rho1;
	dcopy_(n, &work[r * work_dim1 + 1], &c__1cgs, &work[u * work_dim1 + 1], &
		c__1cgs);
	daxpy_(n, &beta, &work[q * work_dim1 + 1], &c__1cgs, &work[u * work_dim1 
		+ 1], &c__1cgs);
...
/* Computing 2nd power */
	d__2 = beta;
	d__1 = d__2 * d__2;
	dscal_(n, &d__1, &work[p * work_dim1 + 1], &c__1cgs);
	daxpy_(n, &beta, &work[q * work_dim1 + 1], &c__1cgs, &work[p * work_dim1 
		+ 1], &c__1cgs);
	daxpy_(n, &c_b6, &work[u * work_dim1 + 1], &c__1cgs, &work[p * work_dim1 
		+ 1], &c__1cgs);
    } else {
	dcopy_(n, &work[r * work_dim1 + 1], &c__1cgs, &work[u * work_dim1 + 1], &
		c__1cgs);
	dcopy_(n, &work[u * work_dim1 + 1], &c__1cgs, &work[p * work_dim1 + 1], &
		c__1cgs);
    }
...
/*        Compute direction adjusting scalar ALPHA. */
    ...
    (*matvec)(&c_b6, &work[phat * work_dim1 + 1], &c_b31, &work[vhat * 
	    work_dim1 + 1]);
    alpha = rho / ddot_(n, &work[rtld * work_dim1 + 1], &c__1cgs, &work[vhat * 
	    work_dim1 + 1], &c__1cgs);

    dcopy_(n, &work[u * work_dim1 + 1], &c__1cgs, &work[q * work_dim1 + 1], &
	    c__1cgs);
    d__1 = -alpha;
    daxpy_(n, &d__1, &work[vhat * work_dim1 + 1], &c__1cgs, &work[q * work_dim1 
	    + 1], &c__1cgs);

/*        Compute direction adjusting vector UHAT. */
/*        PHAT is being used as temporary storage here. */

    dcopy_(n, &work[q * work_dim1 + 1], &c__1cgs, &work[phat * work_dim1 + 1], &
	    c__1cgs);
    daxpy_(n, &c_b6, &work[u * work_dim1 + 1], &c__1cgs, &work[phat * work_dim1 
	    + 1], &c__1cgs);
    (*psolve)(&work[uhat * work_dim1 + 1], &work[phat * work_dim1 + 1]);

/*        Compute new solution approximation vector X. */

    daxpy_(n, &alpha, &work[uhat * work_dim1 + 1], &c__1cgs, &x[1], &c__1cgs);
...
/*        Compute residual R and check for tolerance. */
    ...
    (*matvec)(&c_b6, &work[uhat * work_dim1 + 1], &c_b31, &work[qhat * 
	    work_dim1 + 1]);
    d__1 = -alpha;
    daxpy_(n, &d__1, &work[qhat * work_dim1 + 1], &c__1cgs, &work[r * work_dim1 
	    + 1], &c__1cgs);
    *resid = dnrm2_(n, &work[r * work_dim1 + 1], &c__1cgs) / bnrm2;
